/*This script handles the reading of winter semester and spring semester information from a repository and the assigning of them
to the winter and spring panels*/
...
public class WinterTimetable : MonoBehaviour
{
    public string winterURL = "https://raw.githubusercontent.com/AlviRC/Thesis/refs/heads/main/Excel%20Files/WinterSemester.csv"; // GitHub CSV file URL
    public string springURL = "https://raw.githubusercontent.com/AlviRC/Thesis/refs/heads/main/Excel%20Files/SpringSemester.csv"; // GitHub CSV file URL
    ...
    /*At start, a function is called to read all the data and assign it to the winter and spring panels*/
    void Start()
    {
        StartCoroutine(LoadCSV(winterURL));
    }

    /*The function below handles the reading of the data of from the repository 
    It first reads the winter semester subjects repository and and afterwards the spring semester ones*/
    IEnumerator LoadCSV(string url)
    {
        ...
        GameObject tempTemplate = templateButton;
        GameObject tempPanel = panelToHide;
        ...
        request = UnityWebRequest.Get(url); // Use UnityWebRequest to download data
        yield return request.SendWebRequest(); // Wait for request completion
        Debug.LogError("Does it come2");
        ...
        csvData = request.downloadHandler.text; // Get the CSV text data
        ParseCSV(csvData);

        templateButton.SetActive(false);
        panelToHide.SetActive(false);

        tempTemplate.SetActive(false);
        ...
        //     ParseCSV(csvData);
        // request.Dispose(); // Dispose the first request explicitly
        // Debug.LogError("Does it come");

        // templateButton = templateButton2;
        // scrollContent = scrollContent2;
        // panelToHide = panelToHide2;
        // // url = springURL + "?t=" + UnityEngine.Random.Range(0, 100000);
        // url = springURL;
    ...
    {
        Debug.LogError(panelToHide.name);
        Debug.LogError(templateButton.transform.parent.parent.parent.name);
        Debug.LogError(scrollContent.parent.parent.name);
        ...
        while ((line = reader.ReadLine()) != null)
        {
            ...
            // Use the quoted-aware parser instead of simple split
            string[] fields = ParseCsvLineSimple(line);

            if (fields.Length < 8) continue; // Ensure enough columns
    ...
    /*The function below handles the parsing and orginizing of the read data from the repository to the corresponding panels
    ...
    /*The function below handles the assigning of the read data from the repository to the corresponding panels
    It firsts starts with the winter semester panel and later with the spring semester panels*/
    void PopulateScrollView(List<string[]> data)
    {
        foreach (var fields in data)
        {
            ...
            TMP_Text subjectCodeText = newButton.transform.Find("Subject Code").GetComponent<TMP_Text>();
            TMP_Text courseNameText = newButton.transform.Find("Course Name").GetComponent<TMP_Text>();
            TMP_Text professorsText = newButton.transform.Find("Professors").GetComponent<TMP_Text>();
            TMP_Text mondayText = newButton.transform.Find("Monday").GetComponent<TMP_Text>();
            TMP_Text tuesdayText = newButton.transform.Find("Tuesday").GetComponent<TMP_Text>();
            TMP_Text wednesdayText = newButton.transform.Find("Wednesday").GetComponent<TMP_Text>();
            TMP_Text thursdayText = newButton.transform.Find("Thursday").GetComponent<TMP_Text>();
            ...
            if (subjectCodeText) subjectCodeText.text = fields[0];
