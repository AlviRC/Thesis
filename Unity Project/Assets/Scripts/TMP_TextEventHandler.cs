    ...
    void Update()
    {
        if (Input.GetMouseButtonDown(0) || open == 1)
        {
            ...
            if (linkIndex != -1 || teach == 1)
            {
                ...
                // string clickedWord = linkInfo.GetLinkID(); // The word that was clicked
                Debug.LogError("DOWN here");
                Debug.LogError(clickedWord);
                GameObject targetObject = player;
                // string waypointName = "GameObject - " + room;
                // GameObject targetObject = waypoints.Find(obj => obj.name == waypointName);
                //Auditorium SO
                if (clickedWord == "ΑΜΦ ΣΟ" || clickedWord == "ΑΜΦΣΟ" || clickedWord == "AMF SO" || clickedWord == "AMFSO")
                {
                    Debug.LogError("MPLEEE");
                    targetObject = waypoints.Find(obj => obj.name == "GameObject - AMFSO");
                    Debug.LogError(targetObject.name);
                }
                ...
                {
                    targetObject = waypoints.Find(obj => obj.name == "GameObject - H210");
                }
