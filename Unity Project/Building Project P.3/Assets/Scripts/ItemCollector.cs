/*This script handles the mini game, the old devices museum, the sound settings and the pink dog.*/
...
public class ItemCollector : MonoBehaviour
{
    ...
    public Button Playpausebutton2;
    ...
    void Start()
    {
        ...
        Playpausebutton2.onClick.AddListener(Pause_Play);
    ...
    void Update()
    {
        ...
        //Museum Case
        else if (Physics.Raycast(ray, out hit, 8f, itemLayer2))
        {
            Cursor.visible = true;
            Cursor.lockState = CursorLockMode.None;
            if (Input.GetMouseButtonDown(0))
        ...
        //Pink Dog Case
        else if (Physics.Raycast(ray, out hit, 8f, itemLayer3))
        {
            Cursor.visible = true;
            Cursor.lockState = CursorLockMode.None;
            if (Input.GetMouseButtonDown(0))
    ...
    /*Plays and Pauses the Background Song.*/
    public void Pause_Play()
    {
        if (!audioSource[2]) return;

        // If audio hasn't started yet, start it
        if (!audioSource[2].isPlaying && IsPaused)
        {
            audioSource[2].Play();
            Playpauseimage.sprite = PlayPause[1];
            Playpauseimage2.sprite = PlayPause[1];
        ...
        else
        {
            ...
            Playpauseimage2.sprite = PlayPause[0];
    ...
    /*Changes the volume of the Background Song*/
    ...
    public void Elevator_Sounds(int sound)
    {
        if (sound == 3)
        {
            audioSource[3].Play();
    ...
    /*Provides the final message given when the user reaches the secret location after collecting all mini-game items.*/
